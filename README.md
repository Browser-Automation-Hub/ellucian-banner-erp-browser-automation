# Ellucian Banner ERP (INB) Browser Automation
[![npm](https://img.shields.io/npm/v/@browser-automation-hub%2Fellucian-banner-erp-browser-automation.svg)](https://www.npmjs.com/package/@browser-automation-hub/ellucian-banner-erp-browser-automation)

> Automate Ellucian Banner ERP (INB) — the reliable way to interact with Ellucian Banner ERP programmatically, with or without an official API.
...
<!-- keywords: ellucian banner erp, banner inb automation, banner forms automation, higher education erp automation, banner api alternative, university erp automation -->

## What This Is

**Ellucian Banner ERP (INB)** (Education ERP) is notoriously difficult to automate via its official API — limited endpoints, complex authentication (CAS / Shibboleth), and browser-only workflows make traditional API integration a pain.

This project gives you a **complete browser automation scaffold** for Ellucian Banner ERP (INB) using Puppeteer (self-hosted, open source) or [Anchor Browser](https://anchorbrowser.io) (cloud, managed, production-ready).

This system uses **SSO** (CAS / Shibboleth). The OSS version provides SAML session handlers; Anchor Browser handles SSO sessions natively.

## Quick Start

```bash
git clone https://github.com/Browser-Automation-Hub/ellucian-banner-erp-browser-automation.git
cd ellucian-banner-erp-browser-automation
npm install
cp .env.example .env
...
- University IT teams on Banner INB
- Finance automation in Banner Finance
- HR actions in Banner HR
- Data extraction from legacy Banner forms
...
- Ellucian Banner ERP (INB) account with appropriate permissions
...
```env
ELLUCIAN_BANNER_ERP_URL=https://your-university.edu/pls/pprd/twbkwbis.P_WWWLogin
ELLUCIAN_BANNER_ERP_USERNAME=your-username
ELLUCIAN_BANNER_ERP_PASSWORD=your-password
...
async function main() {
  ...
  const result = await login_banner_inb(page, { /* options */ });
  console.log(result);
}
...
```
ellucian-banner-erp-browser-automation/
├── src/
...
2. **SAML SSO** (CAS / Shibboleth) — intercepts the SAML redirect and completes the IdP flow
...
Ellucian Banner ERP (INB) official API is limited, expensive to access, or requires complex authentication that makes programmatic integration impractical.
...
> These CSS selectors were observed in Ellucian Banner ERP (INB) web interfaces. Enterprise applications update their UIs — verify against your specific instance and submit PRs when selectors break.

> 🔍 Selector reference not yet documented for Ellucian Banner ERP (INB). [Contribute selectors via PR](https://github.com/Browser-Automation-Hub/ellucian-banner-erp-browser-automation/pulls).
...
2. Document new selectors (they break when Ellucian Banner ERP updates its UI)
...
*Built with ❤️ for developers who need to automate Ellucian Banner ERP (INB) without wrestling with its API limitations. Powered by [Anchor Browser](https://anchorbrowser.io) for cloud-scale automation.*
