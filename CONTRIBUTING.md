# Contributing to Ellucian Banner ERP (INB) Browser Automation
...
- **Broken selectors**: Ellucian Banner ERP (INB) updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Ellucian Banner ERP (INB) instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.ELLUCIAN_BANNER_ERP_URL}/path/to/section`);
...
When you find working selectors for Ellucian Banner ERP (INB), document them:

```javascript
// Ellucian Banner ERP (INB) selectors (verified YYYY-MM-DD)
...
Ellucian Banner ERP (INB) updates its UI periodically. When selectors break:
