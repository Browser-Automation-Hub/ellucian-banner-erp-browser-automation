/**
 * anchor-cloud.js — AnchorBrowser cloud example for Ellucian Banner ERP (INB)
 *
...
const { login_banner_inb, navigate_form } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Ellucian Banner ERP (INB) automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_banner_inb(page, {});
      log('Action 1 result:', data1);

      const data2 = await navigate_form(page, {});
